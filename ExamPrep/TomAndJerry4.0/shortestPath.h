...
bool solveMaze(int x, int y, bool** &maze, Point T, Point J, int** sol) {
	...
	if (isValidPlace(x, y, maze, sol) == true) {     //check whether (x,y) is valid or not
		...
		if (solveMaze(x + 1, y, maze, T, J, sol) == true)       //find path by moving right direction
			return true;
		if (solveMaze(x, y + 1, maze, T, J, sol) == true)         //when x direction is blocked, go for bottom direction
			return true;
