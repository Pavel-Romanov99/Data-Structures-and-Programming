...
//prints out the number of turns and the path of directions
void DirectionsAndTurns(string str) {
	string path = "start ";
	int startX = str[0] - '0';
	int startY = str[2] - '0';
	...
	for (int i = 3; i < str.length() - 2; i++)
	{
		if (isNumber(str[i]) && isNumber(str[i + 2])) {
			...
			if (startX == x + 1) path += "W ";
			if (startX == x - 1) path += "E ";
			if (startY == y + 1) path += "N ";
			if (startY == y - 1) path += "S ";

			startX = x;
			startY = y;
		}
	}
	cout << "The drone commands are: " << path << endl;

	int turn_counter = 0;
	for (int i = 6; i < path.size() - 2; i++)
	{
		if (path[i] != path[i + 2]) turn_counter++;
	}
	cout << "Number of turns during the path: " << turn_counter << endl;
	cout << endl;
}

//checks how many points have been painted from a path
int compare(vector<Point>& a, vector<Point>& b) {
	int counter = 0;
...
//make a path to a vector of points 
vector<Point> stringToVector(string str) {
	vector<Point> vector;
...
void check(int TomX, int TomY, int JerryX, int JerryY,
	bool** &maze, std::string& path, int &level, int &width, int &height, vector<Point> &vector1)
{
	...
	if (TomX == JerryX && TomY == JerryY && path.length() == 7 * level + 3) {
		std::cout << path << "\n";
		vector<Point> comparison = stringToVector(path);
		cout << "This path is painted on this many spots: " << compare(vector1, comparison) << endl;
		DirectionsAndTurns(path);
	}
	else {
		path += " -> ";
		maze[TomY][TomX] = 0;
		check(TomX + 0, TomY - 1, JerryX, JerryY, maze, path, level, width, height, vector1);
		check(TomX + 0, TomY + 1, JerryX, JerryY, maze, path, level, width, height, vector1);
		check(TomX - 1, TomY + 0, JerryX, JerryY, maze, path, level, width, height, vector1);
		check(TomX + 1, TomY + 0, JerryX, JerryY, maze, path, level, width, height, vector1);
		maze[TomY][TomX] = 1;
	}
	path.resize(len);
...
void RunProgram()
{
	...
	//display the maze
	cout << "The Room looks like this: " << endl; 
	for (int i = 0; i < length; i++)
	{
		for (int j = 0; j < width; j++)
		{
	...
	check(TomX,  TomY,  JerryX,  JerryY, maze1, path,  level, width, length, painted_spots);
	cout << endl;
